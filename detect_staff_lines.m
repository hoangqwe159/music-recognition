function [ staff_lines ] = detect_staff_lines( bw_image )
% DETECT STAFF LINES 
%   Inputs, binary image
%   Outputs, positions and cluster of the staff lines
    
    % Erosion of short horizontal lines 
...
%     plot(sum(bw_image,2), fliplr(1:size(bw_image,1)));
    ...
    % All lines < max(pks) / 5 will be removed
    tresh = pks > max(pks)/5;
    locs = locs .* tresh;
    pks = pks .* tresh;
    ...
    if mod(length(locs_tresh), 5) == 0
        ...
        staff_lines = locs_tresh;         
        % Remove shortest line until the systems consist of (5 x n) lines
